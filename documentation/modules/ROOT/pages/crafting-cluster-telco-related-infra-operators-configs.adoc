...
These configs may vary from release to release, that's why we create a `{policygen-common-file-ocp-release}` file. We will likely have a common configuration profile for each release we deploy.
...
1. Create the `common` testing PolicyGenerator for {openshift-release} OpenShift Cluster:
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----

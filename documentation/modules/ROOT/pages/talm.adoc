...
Leveraging TALM, we can manage the lifecycle of thousands of clusters at scale in a constrained operational environment. This capability is essential when working in telecom or other environments where service level agreements require managed updates to a fleet of clusters.

Remember that link:ztp-workflow.html[ZTP] generates installation and configuration resources from manifests (`ClusterInstances` and `PolicyGenerators`) stored in Git. These artifacts are applied to a centralized hub cluster where a combination of OpenShift GitOps, Red Hat Advanced Cluster Management, the Assisted Service deployed by the Infrastructure Operator, and the Topology Aware Lifecycle Manager (TALM) use them to install and configure the cluster. See link:ztp-at-scale.html[ZTP components] chapter.
...
spec:
  ...
  remediationStrategy:
    canaries:
    - mno-cnf77 #Defines the clusters for canary updates
...
IMPORTANT: TALM will basically enforce all existing policies that are bound to a cluster that does not have the label "ztp-done'. This is performed by creating automatically a proper CGU CR.
...
A ZTP wave is basically an annotation (ran.openshift.io/ztp-deploy-wave) included on each policy template that will permit TALM to apply policies in an ordered manner. As an example, every link:policygen-deepdive.html#policies-templating[policy] generated from a PolicyGenerator by ZTP incorporates a wave number such as the link:{cnf-features-source-crs}/SriovSubscriptionNS.yaml[SriovSubscriptionNS.yaml]:
...
WARNING: All CRs in the same policy must have the same setting for the ztp-deploy-wave annotation. The default value of this annotation for each CR can be overridden in the PolicyGenerator. Default ztp-deploy-wave value can be found on the proper {cnf-features-source-crs}[source crs].
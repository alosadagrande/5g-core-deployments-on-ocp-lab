...
ZTP provides support for deploying single node clusters, compact or three node clusters, and standard OpenShift clusters. This includes the installation of OpenShift and deployment of the cluster at scale. In this lab, we will focus on Multi-Node OpenShift (MNO) clusters, which was introduced in link:mno-intro.html[Introduction to Multi-Node OpenShift (MNO)].

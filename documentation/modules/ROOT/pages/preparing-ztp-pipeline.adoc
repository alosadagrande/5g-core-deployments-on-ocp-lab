...
The ClusterInstance is fundamental for scaling the management of OpenShift Cluster fleets, as it abstracts the complexity of multiple Kubernetes objects into a unified and simplified definition for cluster deployment. It simplifies this process by providing a unified structure to describe the cluster deployment configuration in a single place. In this {example-mno-clusterinstance-link}[link] you can find an example of a ClusterInstance for a standard OpenShift deployment.

In the comming sections, we will create our own ClusterInstances using the {ai-workflow}[Agent Based Installation] flow to deploy a standard OpenShift Cluster in our environment.
...
└── site-policies
    └── fleet
        ...
        │   │       ├── custom-crs
        │   │       └── reference-crs
        │   ├── v{openshift-intermediate-release}
        │   │   └── source-crs
        │   │       ├── custom-crs
        │   │       └── reference-crs
        │   └── v{openshift-final-release}
        │       └── source-crs
        │           ├── custom-crs
        │           └── reference-crs
        └── testing
...
IMPORTANT: The cluster name for the OpenShift standard cluster that we will be deploying using the agent-based install will be **mno-cluster**. That's why this folder exists in the repository structure that we are creating.
...
mkdir -p site-policies/{fleet/testing/{openshift-release}/source-crs/reference-crs,fleet/testing/{openshift-release}/source-crs/custom-crs}
git add --all
...
-----
+
3. Apply the ZTP GitOps Pipeline configuration:
+
[.console-input]
[source,bash,subs="attributes+,+macros"]
-----
...
* **policies**. It focuses on the logical and operational configuration of those clusters by applying the {rds-config}[Telco Core reference configuration] via RHACM policies.
